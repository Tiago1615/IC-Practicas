...
// Ya no se usan para la triangular, se dejan comentados
//#define INCREMENT 0
//#define DECREMENT 1

// Parámetros de la senoide
#define SINE_STEPS 100
#define SINE_DELAY_MS 10
...
// Ya no se usa, se deja comentado
//unsigned char g_pwmState;

unsigned int g_sineIndex;
...
// Se deja comentada porque ya no se usa
/*
...
{
  Utils.setLED(the_led,the_state);
  return ((the_state==LED_ON)? LED_OFF: LED_ON);
}

// Nueva función: seno discreto por índice
unsigned char nextPWMValue_Sine_Step(unsigned int index)
{
  double angle = 2.0 * PI * index / SINE_STEPS;
  double value = (MAX_PWM_VALUE / 2.0) * (1.0 + sin(angle));

  if (value < MIN_PWM_VALUE) value = MIN_PWM_VALUE;
  if (value > MAX_PWM_VALUE) value = MAX_PWM_VALUE;

  return (unsigned char)(value + 0.5); // redondeo
}
...
{  
  ...
  // Código original, se deja comentado
  //analogWrite(DIGITAL1,g_pwmValue);
  //g_pwmValue = nextPWMValue(g_pwmValue,g_pwmState);

  // Nueva generación senoidal discreta
  g_pwmValue = nextPWMValue_Sine_Step(g_sineIndex);
  analogWrite(DIGITAL1, g_pwmValue);

  g_sineIndex++;
  if(g_sineIndex >= SINE_STEPS) g_sineIndex = 0;
